...
public:
	...
	vector<double> runtime_vector, start_time_vector;
	vector<int> ordering;
	double fitness;
	Data *data;
    ...
    Chromosome(Data *data):
            ...
            runtime_vector(data->task_size, -1), start_time_vector(data->task_size, -1),
            ordering(0), fitness(0), data(data)
    {
    ...
    Chromosome(const Chromosome & other):
            ...
            runtime_vector(other.runtime_vector), start_time_vector(other.start_time_vector),
            ordering(other.ordering), fitness(other.fitness),
            data(other.data), scheduler(other.scheduler),  vm_queue(other.vm_queue) {}
    ...
    void computeFitness(bool check_storage = true, bool check_sequence = false) {
        ...
        if (check_storage && !checkFiles()) { // todo vale a pena lançar um erro aqui?
        ...
        for(auto id_task : ordering){//for each task, do
            if(id_task != data->id_root && id_task != data->id_sink){//if is not root or sink than
                ...
                //update structures
                runtime_vector[id_task] = finish_time;
                ...
                if(id_task == data->id_root)
                    runtime_vector[id_task] = 0;
                else{//sink task
                    double max_value = 0.0;
                    for(auto tk : data->prec.find(id_task)->second)
                        max_value = std::max(max_value, runtime_vector[tk]);
                    runtime_vector[id_task] = max_value;
                }
            }
        }

        fitness = runtime_vector[data->id_sink];
    }
...
private:
    ...
    inline bool checkFiles(){
        ...
        do{
            ...
            if(aux_storage[aux[0]] < 0){//if storage is full, start the file heuristic
                ...
                map_file[old_vm].erase(remove(map_file[old_vm].begin(), map_file[old_vm].end(), min_file), map_file[old_vm].end());
    ...
    inline double ST(Task task, VMachine vm, const vector<double> & queue){
        ...
        for(auto tk : data->prec.find(task.id)->second)
            max_pred_time = std::max(max_pred_time, runtime_vector[tk]);
    ...
    inline double RT(Task task, VMachine vm){
        ...
        for(auto id_file : task.input){
            ...
            auto vm_file = data->vm_map.find(id_vm_file)->second;
						...
						read_time += ceil(TT(file, vm, vm_file));
    ...
    inline double WT(Task task, VMachine vm){
        ...
        for(auto id_file :task.output) {
            ...
            f_queue.first->second.push_back(id_file);
						...
						write_time += ceil(TT(file, vm, vm_file));
